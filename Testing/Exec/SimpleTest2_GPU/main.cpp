...
#pragma omp parallel if (Gpu::notInLaunchRegion())
	...
	//auto  newton_update    = systNU.array(mfi);
        ...
        amrex::Real* csr_x;
        int HP = 0;
        cudaMallocManaged(&csr_row_count, (num_spec+2) * sizeof(int));
        cudaMallocManaged(&csr_col_index, (num_spec+1)*(num_spec+1)*sizeof(int));
        cudaMallocManaged(&csr_val, (num_spec+1)*(num_spec+1)*(box.numPts())*sizeof(amrex::Real));
        cudaMallocManaged(&csr_b, (num_spec+1)*(box.numPts())*sizeof(amrex::Real));
        cudaMallocManaged(&csr_x, (num_spec+1)*(box.numPts())*sizeof(amrex::Real));

        SPARSITY_PREPROC_PRECOND(csr_row_count, csr_col_index, &HP);
