      ...
      char* argv[])
{
    ...
    Real errtol=1e-10;
    int nsubsteps_guess=40;
    int nsubsteps_max=100;
    int nsubsteps_min=2;
    int nsubsteps_final;
...

    {
        ...
        pp.query("rk64_errtol",errtol);
        pp.query("rk64_nsubsteps_guess",nsubsteps_guess);
        pp.query("rk64_nsubsteps_min",nsubsteps_min);
        pp.query("rk64_nsubsteps_max",nsubsteps_max);
	    ...
	    for         (int j = 0; j < len.y; ++j) {
			...
			dt_incr =  dt/ndt;
                        for (int ii = 0; ii < ndt; ++ii) {
                            ...
                            {
                                explicit_react(&nc, &Ncomp, tmp_vect, tmp_src_vect,
                                        tmp_vect_energy, tmp_src_vect_energy,
                                        &dt_incr,&nsubsteps_guess,&nsubsteps_min,
                                        &nsubsteps_max,&nsubsteps_final,&errtol);

                                time=time+dt_incr;
                                nsubsteps_guess=nsubsteps_final;
                            }
