    ...
    do k = lo(3),hi(3)
       do j = lo(2),hi(2)
          do i=lo(1),hi(1)
             ...
             coeff%eos_state(1)%massfrac(1:nspec) = massfrac(i,j,k,1:nspec)
             coeff%eos_state(1)%T = temperature(i,j,k)
             coeff%eos_state(1)%rho = density(i,j,k)
             ...
             mu(i,j,k)  = coeff %  mu(1)
             xi(i,j,k)  = coeff %  xi(1)
             lam(i,j,k) = coeff % lam(1)
             do n=1,nspec
                D(i,j,k,n) = coeff % Ddiag(1,n)
             end do
